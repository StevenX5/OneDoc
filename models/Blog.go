...
type Blog struct {
	...
	//封面地址
	Cover string `orm:"column(cover);size(500)" json:"cover"`
	//最后修改时间
...
// 更新一篇文章
// 2022.9.16
func (blog *Blog) Update(cols ...string) error {
	o := orm.NewOrm()

	blog.BlogTitle = utils.StripTags(blog.BlogTitle)
	temp := NewBlog()
	temp.BlogId = blog.BlogId

	if err := o.Read(temp); err != nil {
		return err
	}
	_, err := o.Update(blog, cols...)
	return err
}

// 增加阅读次数
// 2022.9.17
func (blog *Blog) IncrViewCount(id int) {
	o := orm.NewOrm()
	o.QueryTable(blog.TableNameWithPrefix()).Filter("blog_id", id).Update(orm.Params{
