...
type Itemsets struct {
	...
	ImgUrl			string    `orm:"column(imgurl);size(500)" json:"imgurl"`
...
//查询全部项目列表 add by zhang 2022.9.7
func (item *Itemsets) FindItemsetsAll(pageIndex, pageSize, memberId int) (books []*BookResult, totalCount int, err error) {
	o := orm.NewOrm()
	/*err = item.QueryTable().Filter("item_key", key).One(item)

	if err != nil {
		logs.Error("查询项目空间时出错 ->", err)
		return nil, 0, err
	}*/
	offset := (pageIndex - 1) * pageSize
	//如果是登录用户
...
as t group by book_id) as team 
		...
		count, err1 := o.QueryTable(NewBook().TableNameWithPrefix()).Filter("privately_owned", 0).Count()
