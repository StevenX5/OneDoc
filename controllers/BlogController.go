...
func (c *BlogController) Index() {
	...
	// 增加阅读次数 2022.9.17
	blog.IncrViewCount(blog.BlogId)
...
//上传文章封面
//2022.9.16
func (c *BlogController) UploadCover() {
	...
	blog, err := models.NewBlog().FindByIdentify(identify)

	if err != nil {
		logs.Error("SaveBlog => ", err)
		c.JsonResult(6002, err.Error())
	}
	...
	//附件路径按照项目组织
	// 	filePath := filepath.Join("uploads", blog.BlogIdentify, "images", fileName+ext)
	filePath := filepath.Join(conf.WorkingDirectory, "uploads", blog.BlogIdentify, "images", fileName+ext)
	...
	//生成缩略图并保存到磁盘
	err = graphics.ImageResizeSaveFile(subImg, 500, 300, filePath)
	...
	blog.Cover = conf.URLForWithCdnImage(url)
	...
	logs.Info("用户[", c.Member.Account, "]上传了文章封面 ->", blog.BlogTitle, blog.BlogId, blog.Cover)
